...
Note that the migration *appears* to run on every startup even on a modern configuration, logging
`parameter migrated` for `jenkinsUrl`, `workerUrl` and `driver`. That is not evidence of old data.
Those fields are `transient`, so they are never persisted and are always null on load, and every
guard is therefore always true. The one migration that matters, `jobTemplate`, tests a persisted
field and does not fire on a 0.9.0+ configuration. The value it computes for `driver` is even wrong
for docker templates, since the `image` it consults is transient too.
